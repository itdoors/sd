CREATE TABLE sd_claim (id SERIAL NOT NULL, customer_id INT DEFAULT NULL, types VARCHAR(50), status VARCHAR(50), importance VARCHAR(50), createdAt TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, closedAt TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT NULL, disabled BOOLEAN NOT NULL, discr VARCHAR(255) NOT NULL, targetIndividual_id BIGINT DEFAULT NULL, targetDepartment_id BIGINT DEFAULT NULL, PRIMARY KEY(id));
...
CREATE TABLE org_granted_for_order (id SERIAL NOT NULL, organization_id BIGINT DEFAULT NULL, client_id INT DEFAULT NULL, PRIMARY KEY(id));
CREATE INDEX IDX_6EDD0F2B32C8A3DE ON org_granted_for_order (organization_id);
CREATE INDEX IDX_6EDD0F2B19EB6921 ON org_granted_for_order (client_id);
CREATE TABLE org_granted_for_order_departments (org_granted_for_order_id INT NOT NULL, department_id BIGINT NOT NULL, PRIMARY KEY(org_granted_for_order_id, department_id));
CREATE INDEX IDX_8C06DFD52DA3DEF9 ON org_granted_for_order_departments (org_granted_for_order_id);
CREATE UNIQUE INDEX UNIQ_8C06DFD5AE80F5DF ON org_granted_for_order_departments (department_id);
CREATE TABLE origin_org_for_client (client_id INT NOT NULL, organization_id BIGINT NOT NULL, PRIMARY KEY(client_id, organization_id));
CREATE INDEX IDX_ACBF8BCF19EB6921 ON origin_org_for_client (client_id);
CREATE UNIQUE INDEX UNIQ_ACBF8BCF32C8A3DE ON origin_org_for_client (organization_id);
...
ALTER TABLE org_granted_for_order ADD CONSTRAINT FK_6EDD0F2B32C8A3DE FOREIGN KEY (organization_id) REFERENCES organization (id) NOT DEFERRABLE INITIALLY IMMEDIATE;
ALTER TABLE org_granted_for_order ADD CONSTRAINT FK_6EDD0F2B19EB6921 FOREIGN KEY (client_id) REFERENCES sd_business_role (id) NOT DEFERRABLE INITIALLY IMMEDIATE;
ALTER TABLE org_granted_for_order_departments ADD CONSTRAINT FK_8C06DFD52DA3DEF9 FOREIGN KEY (org_granted_for_order_id) REFERENCES org_granted_for_order (id) NOT DEFERRABLE INITIALLY IMMEDIATE;
ALTER TABLE org_granted_for_order_departments ADD CONSTRAINT FK_8C06DFD5AE80F5DF FOREIGN KEY (department_id) REFERENCES departments (id) NOT DEFERRABLE INITIALLY IMMEDIATE;
ALTER TABLE origin_org_for_client ADD CONSTRAINT FK_ACBF8BCF19EB6921 FOREIGN KEY (client_id) REFERENCES sd_business_role (id) NOT DEFERRABLE INITIALLY IMMEDIATE;
ALTER TABLE origin_org_for_client ADD CONSTRAINT FK_ACBF8BCF32C8A3DE FOREIGN KEY (organization_id) REFERENCES organization (id) NOT DEFERRABLE INITIALLY IMMEDIATE;
