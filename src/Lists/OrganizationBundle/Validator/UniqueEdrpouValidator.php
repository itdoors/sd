        ...
        if ($value) {
            ...
            if (count($conflicts) > 0 && isset($conflicts[0])) {
                ...
                if ($root && $organization) {
                    if (!method_exists($root, 'getId') || (method_exists($root, 'getId') && $root->getId() != $organization->getId())) {
                        $msgString = $this->translator->trans('Organization with edrpou', array(), 'ListsOrganizationBundle').' = %edrpou% '.$this->translator->trans('already exists', array(), 'ListsOrganizationBundle').' '.
